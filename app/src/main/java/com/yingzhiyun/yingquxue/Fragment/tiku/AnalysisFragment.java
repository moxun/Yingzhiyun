...
public class AnalysisFragment extends SimpleFragment {
    ...
    public AnalysisFragment(ExamineBean.ResultBean.DaTiBeanListBean.StemBeanListBean stemBeanListBean, int position, String title, int score, String celect) {
        super();
    ...
    public void initData() throws JSONException {
        ...
        clooseContent.setText(s);
        if (type.equals("RadioSelect")||type.equals("MultiSelect")) {
            ...
            for (int i = 0; i < stemBeanListBean.getRightKey().size(); i++) {
                if (stemBeanListBean.getRightKey().get(i).getContentType().equals("text")) {
                    ...
                    num=num+num3;
                    answer.append(num1);
                    int b= Integer.parseInt(stemBeanListBean.getRightKey().get(i).getWidth()+stemBeanListBean.getRightKey().get(i).getWidth());
        ...
        for (int i = 0; i < stemBeanListBean.getAnalysis().size(); i++) {
            if (stemBeanListBean.getAnalysis().get(i).getContentType().equals("text")) {
                stringBuilder1.append(stemBeanListBean.getAnalysis().get(i).getContent());
            } else if(stemBeanListBean.getAnalysis().get(i).getContentType().equals("latex")){
                stringBuilder1.append("$$");
                stringBuilder1.append(stemBeanListBean.getAnalysis().get(i).getContent());
                stringBuilder1.append("$$");
            }else {
                stringBuilder1.append("<img height=");
                ...
                int b= Integer.parseInt(stemBeanListBean.getAnalysis().get(i).getWidth()+stemBeanListBean.getRightKey().get(i).getWidth());
                stringBuilder1.append(" width="+num+">");
                stringBuilder1.append(stemBeanListBean.getAnalysis().get(i).getContent());
        ...
        if (stemBeanListBean.getOptionsList().size() > 0) {
            analysisCeAdapter = new AnalysisCeAdapter(getContext(), stemBeanListBean.getOptionsList());
